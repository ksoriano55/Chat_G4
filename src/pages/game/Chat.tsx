...
const ChatApp = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([{ id: 0, text: "", sender: "", time: "" }]);
  ...
  useEffect(() => {
    if (messages.length > 0) {
      console.log("Genero BK", messages)
      let save_bk_chat = {
  ...
  const handleSendFile = () => {
    ...
    reader.onload = () => {
      const base64 = reader.result; // ejemplo: data:image/png;base64,iVBOR...

      const data = {
        name: file.name,
        type: file.type,
        data: cifrarMensaje(JSON.stringify(base64)),
        from: localStorage.getItem("user"),
        to: toMessages
      };
      // let fileMessage = cifrarMensaje(JSON.stringify(fileMsg));
      socket.emit("manejar_archivo", data);
    };
    reader.readAsDataURL(file);
    setMessages((messages) => [...messages, { id: messages.length + 1, text:`📎Archivo: ${file.name}`, sender: localStorage.getItem("user") ?? "", time: getHours(new Date) }]);
  };
  return (
    <>
      <Header />
